...
if [ -n $RUBY_VERSION ]; then
  version_opt="--ruby-version=$RUBY_VERSION"
fi

xing new $version_opt $PROJECT_NAME
...
if [ -n $POSTGRES_USER ]; then
  sed -i "s|# username: postgres|username: ${POSTGRES_USER}|" backend/config/database.yml
fi
...
if bundle exec rake spec:full; then
